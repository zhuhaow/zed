...
use gpui2::{Executor, Task, TestAppContext};
...
#[gpui2::test]
async fn test_semantic_index(executor: Executor, cx: &mut TestAppContext) {
    init_test(cx);
...
async fn test_embedding_batching(cx: &mut TestAppContext, mut rng: StdRng) {
    ...
    let mut queue =
        EmbeddingQueue::new(embedding_provider.clone(), Arc::new(cx.executor().clone()));
    for file in &files {
