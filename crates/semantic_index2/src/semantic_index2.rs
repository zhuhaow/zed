...
pub fn init(
    ...
    //                 let previously_indexed = semantic_index.update(cx, |index, cx| {
    ...
    cx.spawn_on_main(move |cx| async move {
        let semantic_index = SemanticIndex::new(
            fs,
            db_file_path,
            Arc::new(OpenAIEmbeddingProvider::new(
                http_client,
                Arc::new(cx.executor().clone()),
            )),
            language_registry,
            cx.clone(),
        )
...
impl SemanticIndex {
    ...
    pub fn authenticate(&mut self, cx: &mut ModelContext<Self>) {
        todo!();
        // if !self.embedding_provider.has_credentials() {
        //     self.embedding_provider.retrieve_credentials(cx).await;
        // } else {
    ...
    pub async fn new(
        ...
        let db =
            VectorDatabase::new(fs.clone(), database_path, Arc::new(cx.executor().clone())).await?;
        ...
        // TODO: I dont really like the cloning going on in this update.
        cx.update(|cx| {
            cx.build_model(|cx| {
                let t0 = Instant::now();
                ...
                let db = db.clone();
                let long_lived_db = db.clone();
                let embedding_queue = embedding_queue.clone();
                let embedding_queue_arc = Arc::new(Mutex::new(embedding_queue.clone()));
                let _embedding_task = cx.spawn(move |_, _|
                {
                    let embedding_queue = embedding_queue.clone();
                    let embedding_files = embedding_queue.finished_files();
                    async move {
                        while let Ok(file) = embedding_files.recv().await {
            ...
            for _ in 0..cx.executor().num_cpus() {
                let fs = fs.clone();
                let mut parsing_files_rx = parsing_files_rx.clone();
                let embedding_provider = embedding_provider.clone();
                let embedding_queue_arc = embedding_queue_arc.clone();
                let executor = cx.executor().clone();
                _parsing_files_tasks.push(cx.spawn(|_, _| async move {
                    let mut retriever = CodeContextRetriever::new(embedding_provider.clone());
                    loop {
                        let mut timer = executor.timer(EMBEDDING_QUEUE_FLUSH_TIMEOUT).fuse();
                        ...
                        futures::select_biased! {
                            ...
                            _ = timer => {
                                embedding_queue_arc.lock().flush();
                            }
                        }
                    }
                }));
    ...
    fn project_entries_changed(
        ...
        if let WorktreeState::Registered(_) = worktree_state {
            cx.spawn(|this, mut cx| async move {
                cx.executor().timer(BACKGROUND_INDEXING_DELAY).await;
    ...
    fn register_worktree(
        ...
        let registration = cx.spawn(move |this, mut cx| {
            async move {
                let register = async {
                    scan_complete.await;
                    ...
                    let worktree = if let Some(project) = project.upgrade() {
                        project
                            .update(&mut cx, |project, cx| {
                                project.worktree_for_id(worktree_id, cx)
                            })?
                            .ok_or_else(|| anyhow!("worktree dropped"))?
                    } else {
                        return anyhow::Ok(());
                    };

                    let worktree = worktree.update(&mut cx, |worktree, _| worktree.snapshot())?;
    ...
    pub fn search_files(
        ...
        cx.spawn(move |this, mut cx| async move {
            let database =
                VectorDatabase::new(fs.clone(), db_path.clone(), Arc::new(cx.executor().clone()))
                    .await?;
            ...
            let batch_n = cx.executor().num_cpus();
            ...
            for batch in file_ids.chunks(batch_size) {
                ...
                if let Some(db) =
                    VectorDatabase::new(fs, db_path.clone(), Arc::new(cx.executor().clone()))
                        .await
                        .log_err()
                {
            ...
            project.update(&mut cx, |project, cx| {
                for (worktree_db_id, file_path, byte_range) in spans {
                    let project_state = if let Some(index) = this.upgrade() {
                        if let Some(state) = index.read(cx).projects.get(&weak_project) {
                            state
                        } else {
                            return Err(anyhow!("project not added"));
                        }
                    } else {
                        return Err(anyhow!("unable to upgrade Semantic Index"));
                    };
    ...
    fn search_modified_buffers(
        ...
        cx.executor().spawn(async move {
            let db = VectorDatabase::new(fs, db_path.clone(), Arc::new(executor)).await?;
    ...
    pub fn index_project(
        ...
        // todo!("authenticate before indexing the project");
        // let authentication = self.authenticate(cx);
