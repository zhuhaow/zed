...
pub struct NumericStepper {
    id: ElementId,
    value: SharedString,
    on_decrement: Box<dyn Fn(&ClickEvent, &mut Window, &mut WindowContext) + 'static>,
    on_increment: Box<dyn Fn(&ClickEvent, &mut Window, &mut WindowContext) + 'static>,
    /// Whether to reserve space for the reset button.
    reserve_space_for_reset: bool,
    on_reset: Option<Box<dyn Fn(&ClickEvent, &mut Window, &mut WindowContext) + 'static>>,
}
...
impl NumericStepper {
    pub fn new(
        ...
        on_decrement: impl Fn(&ClickEvent, &mut Window, &mut WindowContext) + 'static,
        on_increment: impl Fn(&ClickEvent, &mut Window, &mut WindowContext) + 'static,
    ) -> Self {
        Self {
            id: id.into(),
            value: value.into(),
    ...
    pub fn on_reset(
        mut self,
        on_reset: impl Fn(&ClickEvent, &mut Window, &mut WindowContext) + 'static,
    ) -> Self {
