...
use gpui::{
    ...
    transparent_black, Action, AnyView, AnyWeakView, AnyWindowHandle, AppContext, AsyncAppContext,
...
type WorkspaceItemBuilder =
    Box<dyn FnOnce(&mut Window, &mut ViewContext<Pane>) -> Box<dyn ItemHandle>>;
    ...
    builders.push(|project, project_path, window, cx| {
        ...
        Some(cx.spawn(|cx| async move {
            ...
            let build_workspace_item =
                Box::new(|window: &mut Window, cx: &mut ViewContext<Pane>| {
                    Box::new(
                        cx.new_view(|cx| I::for_project_item(project, project_item, window, cx)),
...
struct SerializableItemDescriptor {
    ...
    cleanup: fn(WorkspaceId, Vec<ItemId>, &mut Window, &mut WindowContext) -> Task<Result<()>>,
...
impl Workspace {
    ...
    pub fn new(
        ...
        cx.subscribe_in_window(&project, window, move |this, _, event, window, cx| {
            match event {
        ...
        cx.subscribe_in_window(&center_pane, window, Self::handle_pane_event)
            .detach();
        ...
        let _apply_leader_updates = cx.spawn(|this, mut cx| async move {
            while let Some((leader_id, update)) = leader_updates_rx.next().await {
                Self::process_leader_update(&this, leader_id, update, window_handle, &mut cx)
                    .await
                    .log_err();
            }
        ...
        if let Some(call) = ActiveCall::try_global(cx) {
            let call = call.clone();
            let subscriptions =
                vec![cx.subscribe_in_window(&call, window, Self::on_active_call_event)];
        ...
        let _items_serializer = cx.spawn(|this, mut cx| async move {
            Self::serialize_items(&this, serializable_items_rx, window_handle, &mut cx).await
        });
            ...
            cx.spawn(|workspace, mut cx| async move {
                    ...
                    .with_context(|| format!("Navigating to {project_path:?}"))
                {
                    Ok((project_entry_id, build_item)) => {
                        ...
                        pane.update_in_window(window_handle, &mut cx, |pane, window, cx| {
                    ...
                    Err(open_by_project_path_e) => {
                        ...
                        if let Some(abs_path) = abs_path {
                            ...
                            let open_by_abs_path = workspace.update_in_window(window_handle, &mut cx, |workspace, window, cx| {
    ...
    pub fn prepare_to_close(
        ...
        cx.spawn(|this, mut cx| async move {
            ...
            if let Some(active_call) = active_call {
                ...
                {
                    let answer = window_handle.update(&mut cx, |_, _, cx| {
                        cx.prompt(
                            PromptLevel::Warning,
    ...
    fn save_all_internal(
        ...
        cx.spawn(|workspace, mut cx| async move {
            let dirty_items = if save_intent == SaveIntent::Close && !dirty_items.is_empty() {
                let (serialize_tasks, remaining_dirty_items) = workspace.update_in_window(
                    window_handle,
                    &mut cx,
                    |workspace, window, cx| {
    ...
    pub fn close_all_docks(&mut self, window: &mut Window, cx: &mut ViewContext<Self>) {
    ...
    pub fn focus_panel<T: Panel>(
        ...
        let panel = self.focus_or_unfocus_panel::<T>(window, cx, |_, _| true)?;
    ...
    pub fn toggle_panel_focus<T: Panel>(
        ...
        self.focus_or_unfocus_panel::<T>(window, cx, |panel, cx| {
    ...
    fn add_pane(&mut self, window: &Window, cx: &mut ViewContext<Self>) -> View<Pane> {
        ...
        T: ProjectItem,
    {
        ...
        let item =
            cx.new_view(|cx| T::for_project_item(self.project().clone(), project_item, window, cx));
    ...
    pub fn start_following(
        ...
        Some(cx.spawn(|this, mut cx| async move {
            ...
            this.update_in_window(window, &mut cx, |this, window, cx| {
    ...
    pub fn follow(&mut self, leader_id: PeerId, window: &mut Window, cx: &mut ViewContext<Self>) {
    ...
    async fn process_leader_update(
        ...
        this.update_in_window(window_handle, cx, |this, window, cx| {
            this.leader_updated(leader_id, window, cx)
        })?;
        Ok(())
    }
    ...
    fn remove_from_session(&mut self, window: &Window, cx: &mut WindowContext) -> Task<()> {
    ...
    fn serialize_workspace(&mut self, window: &Window, cx: &mut ViewContext<Self>) {
    ...
    fn serialize_workspace_internal(&self, window: &Window, cx: &mut WindowContext) -> Task<()> {
    ...
    async fn serialize_items(
        ...
        while let Some(items_received) = serializable_items.next().await {
            ...
            for (_, item) in unique_items.into_iter() {
                if let Ok(Some(task)) =
                    this.update_in_window(window_handle, cx, |workspace, window, cx| {
    ...
    pub(crate) fn load_workspace(
        ...
        cx.spawn(|workspace, mut cx| async move {
            ...
            let clean_up_tasks =
                workspace.update_in_window(window_handle, &mut cx, |_, window, cx| {
                    item_ids_by_kind
                        .into_iter()
    ...
    fn actions(&self, div: Div, cx: &mut ViewContext<Self>) -> Div {
        self.add_workspace_actions_listeners(div, cx)
            .on_action(cx.listener2(Self::close_inactive_items_and_panes))
            .on_action(cx.listener2(Self::close_all_items_and_panes))
            .on_action(cx.listener2(Self::save_all))
            .on_action(cx.listener2(Self::send_keystrokes))
            .on_action(cx.listener2(Self::add_folder_to_project))
            .on_action(cx.listener2(Self::follow_next_collaborator))
            .on_action(cx.listener2(Self::close_window))
            .on_action(cx.listener2(Self::activate_pane_at_index))
            .on_action(cx.listener2(Self::move_item_to_pane_at_index))
            .on_action(cx.listener2(|workspace, _: &Unfollow, window, cx| {
                let pane = workspace.active_pane().clone();
                workspace.unfollow_in_pane(&pane, cx);
            }))
            .on_action(cx.listener2(|workspace, action: &Save, window, cx| {
                workspace
                    .save_active_item(action.save_intent.unwrap_or(SaveIntent::Save), cx)
                    .detach_and_prompt_err("Failed to save", cx, |_, _| None);
            }))
            .on_action(
                cx.listener2(|workspace, _: &SaveWithoutFormat, window, cx| {
                    workspace
                        .save_active_item(SaveIntent::SaveWithoutFormat, cx)
                        .detach_and_prompt_err("Failed to save", cx, |_, _| None);
                }),
            )
            .on_action(cx.listener2(|workspace, _: &SaveAs, window, cx| {
                workspace
                    .save_active_item(SaveIntent::SaveAs, cx)
                    .detach_and_prompt_err("Failed to save", cx, |_, _| None);
            }))
            .on_action(
                cx.listener2(|workspace, _: &ActivatePreviousPane, window, cx| {
                    workspace.activate_previous_pane(cx)
                }),
            )
            .on_action(cx.listener2(|workspace, _: &ActivateNextPane, window, cx| {
                workspace.activate_next_pane(window, cx)
            }))
            .on_action(
                cx.listener2(|workspace, action: &ActivatePaneInDirection, window, cx| {
            ...
            .on_action(
                cx.listener2(|workspace, action: &SwapPaneInDirection, window, cx| {
                    workspace.swap_pane_in_direction(action.0, cx)
                }),
            )
            .on_action(cx.listener2(|this, _: &ToggleLeftDock, window, cx| {
            ...
            .on_action(
                cx.listener2(|workspace: &mut Workspace, _: &CloseAllDocks, window, cx| {
            ...
            .on_action(cx.listener2(Workspace::toggle_centered_layout))
    }

    #[cfg(any(test, feature = "test-support"))]
    pub fn test_new(project: Model<Project>, window: &Window, cx: &mut ViewContext<Self>) -> Self {
        use node_runtime::NodeRuntime;
        use session::Session;
    ...
    pub fn register_action<A: Action>(
        ...
        self.workspace_actions.push(Box::new(move |div, cx| {
            ...
            div.on_action(cx.listener2(move |workspace, event, window, cx| {
    ...
    pub fn register_action2<A: Action>(
        ...
        self.workspace_actions.push(Box::new(move |div, cx| {
            let callback = callback.clone();
            div.on_action(cx.listener2(move |workspace, event, _window, cx| {
    ...
    pub fn for_window(window: &Window, cx: &mut WindowContext) -> Option<View<Workspace>> {
                        ...
                        .child(
                            div()
                                ...
                                .when(self.zoomed.is_none(), |this| {
                                    this.on_drag_move(cx.listener2(
...
mod tests {
    ...
    use fs::FakeFs;
    use gpui::{
        px, AnyWindowHandle, DismissEvent, Empty, EventEmitter, FocusHandle, FocusableView, Render,
        TestAppContext, UpdateGlobal, VisualTestContext,
    };
    ...
    async fn test_close_window(cx: &mut TestAppContext) {
        ...
        let task = workspace.update_in_window(window, cx, |workspace, window, cx| {
        ...
        let task = workspace.update_in_window(window, cx, |w, window, cx| {
    ...
    async fn test_prompting_to_save_only_on_last_item_for_entry(cx: &mut TestAppContext) {
        ...
        let left_pane = workspace.update_in_window(window, cx, |workspace, window, cx| {
    ...
    async fn test_pane_navigation(cx: &mut gpui::TestAppContext) {
        ...
        workspace
            .update_in_window(window, cx, |workspace, window, cx| {
    ...
    async fn test_toggle_docks_and_panels(cx: &mut gpui::TestAppContext) {
        ...
        workspace.update_in_window(window, cx, |workspace, window, cx| {
    ...
    async fn test_join_pane_into_next(cx: &mut gpui::TestAppContext) {
        ...
        let top_pane_id = workspace.update_in_window(window, cx, |workspace, window, cx| {
        ...
        let bottom_pane_id = workspace.update_in_window(window, cx, |workspace, window, cx| {
        ...
        let left_pane_id = workspace.update_in_window(window, cx, |workspace, window, cx| {
        ...
        let right_pane_id = workspace.update_in_window(window, cx, |workspace, window, cx| {
    ...
    fn split_pane(
        ...
        return workspace.update_in_window(window_handle, cx, |workspace, window, cx| {
    ...
    async fn test_join_all_panes(cx: &mut gpui::TestAppContext) {
        ...
        split_pane(cx, window, &workspace);
        add_an_item_to_active_pane(cx, &workspace, 2);
        split_pane(cx, window, &workspace); // empty pane
        split_pane(cx, window, &workspace);
